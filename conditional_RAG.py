...
def academic_rag_node(state: State) -> dict:
    """Retrives relevant chunks from the academics handbook."""
...
def fee_rag_node(state: State) -> dict:
    """Retrives relevant chunks from the fee structure PDF."""
    query = state["messages"][-1].content
...
def response_node(state: State) -> dict:
    ...
    if context == "NO RETRIEVAL NEEDED":
        prompt = (
            ...
            f"Answer this question using your own general knwoledge:\n\n{query}"
        )
    else:
        prompt = (
...
while True:
    ...
    result = app.invoke(
        {"programm": student_programme, "messages": [("human", user_query)]}
    )
